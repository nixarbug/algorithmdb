...
/**
 * REST controller for managing {@link com.algorithmdb.domain.BlogEntry}.
 */
@RestController
@RequestMapping("/api")
public class BlogEntryResource {
    ...
    public ResponseEntity<List<BlogEntry>> getAllBlogEntries(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        ...
        Page<BlogEntry> page;
        if (eagerload) {
            page = blogEntryService.findAllWithEagerRelationships(pageable);
        } else {
            page = blogEntryService.findAll(pageable);
        }
    ...
    public ResponseEntity<BlogEntry> getBlogEntry(@PathVariable Long id) {
        ...
        Optional<BlogEntry> blogEntry = blogEntryService.findOne(id);
